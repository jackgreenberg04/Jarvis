...
def get_current_wallpaper() -> str:
    """Returns the current wallpaper path (macOS)."""
    try:
        if sys.platform == "darwin":
            result = subprocess.run(
                ["osascript", "-e",
                 'tell application "Finder" to get POSIX path of (get desktop picture)'],
                capture_output=True, text=True, timeout=5
            )
            val = result.stdout.strip()
            return f"Current wallpaper: {val}" if val else "Could not retrieve wallpaper path."
        else:
