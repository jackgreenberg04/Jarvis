...
MAC_COMMAND_MAP = [
    ...
    (["desktop files", "files on desktop"],
     f'ls -lh "{Path.home() / "Desktop"}"', False),
    (["downloads", "files in downloads"],
     f'ls -lh "{Path.home() / "Downloads"}"', False),
    (["large files", "biggest files", "largest files"],
     f'find {Path.home()} -type f -exec du -sh {{}} + 2>/dev/null | sort -rh | head -10', False),
]
WIN_COMMAND_MAP = MAC_COMMAND_MAP  # alias for compatibility

def _find_hardcoded(task: str) -> str | None:
    task_lower = task.lower()
    
    if "textedit" in task_lower or "notepad" in task_lower or any(ext in task_lower for ext in [".txt", ".log", ".md", ".csv"]):
...
def _ask_gemini(task: str) -> str:
    try:
        ...
        prompt = (
            ...
            f"Target OS: macOS (Apple Silicon M3). If unsafe or impossible, output: UNSAFE\n\n"
            f"Request: {task}\n\nCommand:"
        )
...
def cmd_control(
    ...
    if any(x in command.lower() for x in ["open -a", "open -e", "open ~/"]):
